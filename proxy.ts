...
export function proxy(request: NextRequest) {
  ...
  // Host-based 301 redirect: out-thedoor.com → bestoutthedoor.com. Defense
  // in depth for the platform edge — any request reaching either host is
  // forwarded to the canonical origin with path + query preserved. /health
  // is exempted so the deploy probe still returns 200 on whichever host it
  // lands on.
  if (request.headers.get('host') === 'out-thedoor.com' && request.nextUrl.pathname !== '/health') {
    const target = new URL(
      request.nextUrl.pathname + request.nextUrl.search,
      'https://bestoutthedoor.com',
    );
