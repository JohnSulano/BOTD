...
## Dealer Dashboard (this app)

The buyer-side `/quote` form and the dealer-side `/dealer/quotes` dashboard
share the same `QuoteRequest` model. New buyer submissions land in the
database; dealers who have applied and been approved can sign in at
`/dealer/login` (single email field — no password, no magic link) and see
every incoming request with the configured vehicle, trade-in spec sheet,
buyer ZIP, phone, and submitted timestamp.

Approving a dealer today requires a single manual SQL UPDATE — there is no
admin surface yet:

```sql
UPDATE "DealerApplication"
SET status = 'approved'
WHERE email = 'rep@dealership.com';
```

Until `DealerApplication.status` is set to `'approved'` for at least one
row, `/dealer/login` will accept the email and return the standard
"couldn't find an approved dealer account" message. There is no separate
admin UI in this iteration; flipping status is done in the database.
